        ...
        async function fetchAndDisplayData() {
            try {
                const response = await fetch('https://api.khaliddesu.my.id/api/data');
                if (!response.ok) {
                    throw new Error('Gagal mengambil data');
                }
                const data = await response.json();
                document.getElementById('jumlah-user').textContent = `${data.jumlahUser}`;
                document.getElementById('jumlah-pengikut').textContent = `${data.jumlahPengikut}`;
                document.getElementById('jumlah-money').textContent = `${data.jumlahMoney}`;
                document.getElementById('jumlah-vistor').textContent = `${data.jumlahVistor}`;
            } catch (error) {
